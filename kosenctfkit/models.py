...
class Notification(Base):
    ...
    title = Column(Text)
    content = Column(Text)
    date = Column(Integer, default=lambda: datetime.utcnow().timestamp())
...
class Challenge(Base):
    ...
    files = relationship(
        "Attachment", backref=backref("challenge", cascade="all, delete")
    )
...
class User(Base, UserMixin):
    ...
    created_at = Column(Integer, default=lambda: datetime.now().timestamp())
...
class Team(Base):
    ...
    members = relationship(
        "User", backref=backref("team", cascade="all, delete"), lazy="dynamic"
    )
    ...
    created_at = Column(Integer, default=lambda: datetime.utcnow().timestamp())
...
class Submission(Base):
    ...
    def __repr__(self):
        return "<Submission {}, {}, {}, {}>".format(
            self.user, self.challenge, self.text
        )
