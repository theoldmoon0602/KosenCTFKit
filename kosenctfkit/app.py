...
class App:
    ...
    @property
    def timezone(self):
        return pytz(self.getConfig("timezone", "Asia/Tokyo"))
    ...
    def initCTF(
        self, ctf_name, timezone, start_at, end_at, team_size, score_exp, admin_pass
    ):
        ...
        self.session.add(
            User(name="admin", password=admin_pass, admin=True, hidden=True)
        )
    ...
    def insertSubmission(self, submission, user, challenge):
        ...
        solvedby_user_or_team = (
            self.session.query(Submission)
            .filter(
                Submission.challenge_id == challenge.id,
                Submission.valid == True,
                or_(Submission.user_id == user.id, Submission.team_id == user.team.id),
            )
            .first()
        )
    ...
    def getChallengeById(self, id):
        return self.session.query(Challenge).filter(Challenge.id == id).first()
    ...
    def allTeams(self):
        return (
            ...
            .filter(Team.hidden == False, Team.valid == True)
            .all()
        )
    ...
    def challengeScores(self, challenges):
        ...
        cid_score_counts = (
            self.session.query(Challenge.id, Challenge.score, func.count(Submission.id))
            .group_by(Challenge.id)
            .filter(Submission.valid == True)
            .all()
        )
        ...
        for x in cid_score_counts:
            ...
            if cid in solved_cids:
                scores[cid] = int(eval(exp))

        for c in challenges:
            if c.id not in scores:
                V = c.score
                N = 0
                scores[c.id] = int(eval(exp))
        return scores
    ...
    def allUserScores(self):
        users = self.allUsers()
        submissions = (
            self.session.query(Submission).filter(Submission.valid == True).all()
        )
    ...
    def allTeamScores(self):
        ...
        values = {
            t.id: {"id": t.id, "name": t.name, "score": 0, "last_submission": 0}
            for t in teams
        }
        ...
        chal_scores = {}
        for c in challenges:
            if c.id in chal_solves:
                N = chal_solves[c.id]
                V = c.score
                chal_scores[c.id] = int(eval(exp))

        for s in submissions:
            values[s.team_id]["score"] += chal_scores[s.challenge_id]
            values[s.team_id]["last_submission"] = max(
                values[s.team_id]["last_submission"], s.created_at
            )
        return values
